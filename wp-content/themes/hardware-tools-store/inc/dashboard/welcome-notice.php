 ...
 */
if (!function_exists('hardware_tools_store_is_plugin_installed')) {
    function hardware_tools_store_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('hardware_tools_store_is_plugin_activated')) {
    function hardware_tools_store_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_hardware_tools_store_install_and_activate_plugins', 'hardware_tools_store_install_and_activate_plugins');
add_action('wp_ajax_nopriv_hardware_tools_store_install_and_activate_plugins', 'hardware_tools_store_install_and_activate_plugins');
add_action('wp_ajax_hardware_tools_store_rplugin_activation', 'hardware_tools_store_rplugin_activation');
add_action('wp_ajax_nopriv_hardware_tools_store_rplugin_activation', 'hardware_tools_store_rplugin_activation');
...
function hardware_tools_store_all_plugins_active() {
...
function hardware_tools_store_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('hardware_tools_store_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_hardware_tools_store_plugin_installed($plugin_slug)) {
...
function is_hardware_tools_store_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_hardware_tools_store_dismissed_notice_handler', 'hardware_tools_store_ajax_notice_dismiss_function');

function hardware_tools_store_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'hardware_tools_store_welcome_nonce')) {
...
function hardware_tools_store_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        ...
        if ($hardware_tools_store_current_screen && $hardware_tools_store_current_screen->id !== 'appearance_page_hardware-tools-store-guide-page') {
            ...
            <div class="hardware-tools-store-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="hardware-tools-store-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    ...
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" id="install-activate-button" href="#"><?php _e('Begin Installation', 'hardware-tools-store'); ?></a>`
                    </div>
                </div>
            </div>
            <?php
